...
process FILTER_READS_BASED_ON_READ_LENGTHS_AND_OFFSETS {
    ...
    publishDir "${params.riboseq_dir}/filter_reads_based_on_read_lengths_and_offsets", mode: 'copy'
...
workflow RIBOSEQ_PIPE {
	...
	// map reads to rRNA, continue with unmapped reads
	  (other_genes_mapped_sam, other_genes_unmapped_fasta) = MAP_rRNA_SEGEMEHL(
																pro_filtered_fasta,
											     				other_RNAs_index,
											     				other_RNAs_sequence_ch)


      overrepresented_sequences_other = COUNT_OVERREPRESENTED_SEQUENCES_OTHER(
											other_genes_mapped_sam,
											params.find_overrepresented_sequences_script)

	// map the filtered reads to the transcriptome
      (transcripts_mapped_sam, transcripts_unmapped_fasta) = MAP_TRANSCRIPTOME_SEGEMEHL(
																other_genes_unmapped_fasta,
																transcriptome_index,
																longest_pc_transcript_per_gene_fa)

      (star_mapped_sam, star_unmapped_fasta) = MAP_GENOME_STAR(
													other_genes_unmapped_fasta,
													genome_index,
													gtf_ch)
	  ...
	  (star_mapped_unique_sorted_bam, star_mapped_unique_sorted_bam_bai, bam_sort_index_folder) = SAM_TO_BAM_SORT_AND_INDEX_STAR(star_mapped_sam)

      READ_LENGTH_HISTOGRAM(  transcripts_mapped_unique_sam,
							  params.plot_read_lengths_script  )
      
      alignment_offset_json = DETERMINE_P_SITE_OFFSET(
								bam_bai_folder,
								transcript_id_gene_id_CDS,
